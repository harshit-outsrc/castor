...
from lambda_functions.canvas_events.events.submission_events import SubmissionCreatedEvent, SubmissionUpdatedEvent
...
class TestCanvasEventSubmission(unittest.TestCase):
    def setUp(self):
    ...
    def test_submission_created_event(self):
        test_event = test_events["submission_created"]
        handler = SubmissionCreatedEvent(
    ...
    def test_submission_updated_event(self):
        test_event = test_events["submission_updated"]
        handler = SubmissionUpdatedEvent(
            event=test_event, psql_engine=self.psql_engine, sf_client=self.sf_client, canvas_client=self.canvas_client
        )
        handler.psql_services = self.psql_engine
        handler.sf_services = self.sf_client
