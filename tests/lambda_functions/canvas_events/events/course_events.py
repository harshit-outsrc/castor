...
from lambda_functions.canvas_events.events.course_events import CourseCompletedEvent, CourseProgressEvent
...
class TestCanvasEventCourse(unittest.TestCase):
    def setUp(self):
    ...
    def test_course_completed_event(self):
        test_event = test_events["course_completed"]
        handler = CourseCompletedEvent(
    ...
    def test_course_progress_event(self):
        test_event = test_events["course_progress"]
        handler = CourseProgressEvent(
            event=test_event, psql_engine=self.psql_engine, sf_client=self.sf_client, canvas_client=self.canvas_client
        )
        handler.psql_services = self.psql_engine
        handler.sf_services = self.sf_client
